...
function getSpiderStatus() {
    ...
    $.ajax({
        url: "http://" + ip + "/KG/RDFTDB/getClassState?no=" + new Date().getTime(),
        // url:"a.json",
        type: 'get',
        async: false,
        dataType: 'json',
                        ...
                        'click .start': function (e, value, row, index) {
                            ...
                            if (row.Store == '未开始') {
                                ...
                                $http.get("http://" + ip + "/KG/RDFTDB/store?temp=" + new Date().getTime() + "&ClassName=" + row.ClassName).success(function () {
                                    getSpiderStatus();
                                    $("#myModalContent").html('<span class="badge" style="background-color:red;font-size: 18px">该课程已完成存储！请提交！</span>');
                                })
                                getSpiderStatus();
                                return
                            }
                            else if (row.Store == '正在进行') {
